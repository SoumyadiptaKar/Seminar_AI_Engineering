...
def run_once(config_path: str) -> Dict[str, Any]:
    ...
    unlearn_cfg = UnlearningConfig(
        ...
        extra={
            ...
            "train_batch": run_cfg.get("train_batch", 1),
            ...
            "gd_cycles": cfg.get("gradient_difference", {}).get("cycles", 2),
            "gd_forget_epochs_per_cycle": cfg.get("gradient_difference", {}).get("forget_epochs_per_cycle", 1),
            "gd_retain_epochs_per_cycle": cfg.get("gradient_difference", {}).get("retain_epochs_per_cycle", 1),
            "gd_dry_run": cfg.get("gradient_difference", {}).get("dry_run", False),
            "scrub_forget_epochs": cfg.get("scrub", {}).get("forget_epochs", 1),
            "scrub_retain_epochs": cfg.get("scrub", {}).get("retain_epochs", 2),
            "scrub_retain_lr": cfg.get("scrub", {}).get("retain_lr", run_cfg.get("learning_rate", 1e-4) * 0.5),
            "scrub_pseudo_conf": cfg.get("scrub", {}).get("pseudo_conf", 0.35),
            "scrub_use_pseudo": cfg.get("scrub", {}).get("use_pseudo", False),
            "scrub_dry_run": cfg.get("scrub", {}).get("dry_run", False),
            "ssd_alpha": cfg.get("ssd", {}).get("alpha", 0.2),
            "ssd_target_keywords": cfg.get("ssd", {}).get("target_keywords", ["cv3", "cls"]),
            "ssd_forget_epochs": cfg.get("ssd", {}).get("forget_epochs", 1),
            "ssd_retain_recovery_epochs": cfg.get("ssd", {}).get("retain_recovery_epochs", 1),
            "ssd_dry_run": cfg.get("ssd", {}).get("dry_run", False),
            "sisa_shards": cfg.get("sisa", {}).get("shards", 10),
            "sisa_slices_per_shard": cfg.get("sisa", {}).get("slices_per_shard", 5),
            "sisa_slice_epochs": cfg.get("sisa", {}).get("slice_epochs", 1),
            "sisa_dry_run": cfg.get("sisa", {}).get("dry_run", False),
        },
    )
